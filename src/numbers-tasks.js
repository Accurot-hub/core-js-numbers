...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorLenghtOne = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorLenghtTwo = Math.sqrt(x2 ** 2 + y2 ** 2);
  const productOfLenghtVector = vectorLenghtOne * vectorLenghtTwo;
  const angle = Math.acos(dotProduct / productOfLenghtVector);
  return angle;
}
...
function getParallelepipedDiagonal(a, b, c) {
  const diagonalParallelepiped = a ** 2 + b ** 2 + c ** 2;
  return Math.sqrt(diagonalParallelepiped);
}
...
function getSumOfDigits(num) {
  const arrNum = Array.from(String(num));
  let sum = 0;
